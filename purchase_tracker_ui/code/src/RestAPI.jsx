...
async function api_patch(url, data) {
  return await api_call(url, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
}
...
async function api_call(url, settings = {}) {
  ...
  return await fetch(url, mergeSettings(defaults, settings))
}

const Ex = { api_get, api_post, api_put, api_delete, api_patch }
export default Ex